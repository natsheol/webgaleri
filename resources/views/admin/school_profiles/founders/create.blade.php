...
<div class="bg-white rounded-lg shadow-sm p-6">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-2">Create New Founder</h1>
        <p class="text-gray-600">Add a new founder to the school profile</p>
    </div>
    ...
    <form action="{{ route('admin.school-profile.founders.store') }}" 
          method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        
        {{-- Hidden field for school profile ID --}}
        <input type="hidden" name="school_profile_id" value="{{ $profiles->first()->id ?? 1 }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="birth_year" id="birth_year" value="{{ old('birth_year') }}"
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                      ...
                      placeholder="Enter founder's description...">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="bg-gray-50 p-6 rounded-lg">
            ...
            <div class="p-4 bg-blue-50 border border-blue-200 rounded-md">
                <p class="text-sm text-blue-700">Upload a photo to display the founder's image. Supported formats: JPG, PNG, WEBP. Maximum file size: 4MB.</p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end space-x-4 pt-6 border-t">
            ...
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">
                <i class="fas fa-plus mr-2"></i>Create Founder
            </button>
        </div>
    </form>
</div>
