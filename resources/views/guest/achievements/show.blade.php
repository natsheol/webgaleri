...
<div class="container mx-auto px-6 pt-32 pb-12">
    <div class="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-8">
        ...
        {{-- Image --}}
        @if (!empty($achievement->image) && file_exists(public_path('storage/' . $achievement->image)))
        ...
        {{-- Title --}}
        <h1 class="text-3xl font-bold text-amber-800 mb-2">{{ $achievement->title }}</h1>
        <p class="text-sm text-gray-500 mb-6">
            {{ $achievement->writer ?? 'Admin' }} • {{ $achievement->created_at->format('d M Y') }}
        </p>
        ...
        <div class="mt-6 flex space-x-4">
            ...
            <button onclick="navigator.clipboard.writeText('{{ request()->fullUrl() }}'); alert('Link copied!')" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadLikeStatus() {
        fetch(`/guest/achievements/${achievementId}/like-status`)
            .then(res => res.json())
            .then(data => {
    ...
    likeButton.addEventListener('click', function() {
        fetch(`/guest/achievements/${achievementId}/like`, {
            method: 'POST',
            headers: {
    ...
    function loadComments() {
        fetch(`/guest/achievements/${achievementId}/comments`)
            .then(res => res.json())
            .then(data => {
                if (data.success) {
