...
<div x-data="galleryPage()" x-cloak class="pt-28 pb-20 px-6">
    <div class="max-w-7xl mx-auto transition-all duration-700"
        ...
        <div x-show="!selectedCategory"
            ...
            <template x-for="cat in categories" :key="cat.id">
                <div @click="selectCategory(cat.id)" class="cursor-pointer group relative rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-400 hover:scale-105">
        ...
        <div x-show="selectedCategory"
            ...
            <div class="mt-6 text-center">
                <button @click="selectedCategory = null"
                        class="px-5 py-2 bg-gradient-to-r from-[#b03535] via-[#3c5e5e] to-[#425d9e] text-white rounded-full hover:opacity-95 transition">
                    ← Back to Categories
                </button>
            </div>
        </div>
            ...
            <div class="md:w-2/5 w-full p-5 flex flex-col">
                ...
                <div class="border-t mt-3 pt-3 flex-1 overflow-y-auto space-y-3">
                    ...
                    <template x-for="c in comments" :key="c.id">
                        <div class="text-sm">
                            <strong x-text="c.name || 'Guest'" class="text-gray-800"></strong>
...
function galleryPage() {
    return {
        ...
        openModal(i) {
            ...
            this.modalOpen = true;
            this.loadPhotoState(); // ambil like & comment
        },
        closeModal() {
        ...
        async loadPhotoState() {
            ...
            try {
                ...
                if (likeRes.ok) {
                    ...
                    this.likeCount = data.like_count ?? 0; // <-- consistent key
                }
        ...
        async postComment() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    // reload comments from DB to ensure name & order are consistent
                    await this.loadPhotoState();
                    this.newComment = '';
                } else {
                    // handle validation errors optionally
